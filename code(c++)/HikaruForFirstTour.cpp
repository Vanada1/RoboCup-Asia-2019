...
using namespace std;
void Game1_HikaruForFirstTour::setup(void)
{
	srand(Time);
	system("cls");
	UserGame1::setup();
...
void Game1_HikaruForFirstTour::loop()
{
	ProcessingTime pt;
	pt.start();
    ...
    || (loaded_objects[SUPER_LOADED_ID] > 0 && loaded_objects[RED_LOADED_ID] > 0 && loaded_objects[CYAN_LOADED_ID] > 0 && loaded_objects[BLACK_LOADED_ID] > 0)))
	{
		process = 0;
		large_process = -1;
		process_times = 0;
    ...
    else if(superarea_process_times < 2)
         {
            if (superarea_process_times == 0)
            {
                if (GoInDots(180, 36, 30, 30, POINT_SUPERAREA))
                {
                    if (process_times >= 4)
                    {
                ...
                {
                    if (GoInDots(180, 240, 30, 30, POINT_SUPERAREA))
                    {
                        if (process_times >= 3)
                        {
                            superarea_process_times++;
	...
	else
	{
		...
		{
            if(findObjectInfo(CYAN_LOADED_ID))
            {
            ...
            {
                //randomDots();
                //GoToDots(rnd()%360,rnd()%270,20,20);
                if (Time % 10 == 0 || unknownX == -1)
                {
                    unknownX = rnd() % 360;
                    unknownY = rnd() % 270;
                    process_times = 0;
                ...
                else 
                {
                    ...
                    GoToDots(unknownX,unknownY,20,20);
            }
        }
		
		else if (loaded_objects[RED_LOADED_ID] < kBorderSameObjNum)
		{
            if(findObjectInfo(RED_LOADED_ID))
            {
		...
		{
            if(findObjectInfo(BLACK_LOADED_ID))
            {
...
void Game1_HikaruForFirstTour::CheckNowDot(void)
{
...
int Game1_HikaruForFirstTour::GoToPosition(int x, int y, int wide_decide_x, int wide_decide_y, int wide_judge_arrived)
{
...
void Game1_HikaruForFirstTour::InputDotInformation(void)
{
...
int Game1_HikaruForFirstTour::GoToDot(int x, int y)
{
	// printf("%d %d\n", x * kSize, y * kSize);
...
int Game1_HikaruForFirstTour::HowManyCurved(int id)
{
	/*
...
void Game1_HikaruForFirstTour::GoToAngle(int angle, int distance)
{
...
void Game1_HikaruForFirstTour::saveColorInfo(void)
{
	/*
	最上位
	POINT_DEPOSIT
...
void Game1_HikaruForFirstTour::calculateWallPosition(void)
{
	if (PositionX != -1)
	{
...
void Game1_HikaruForFirstTour::setObjectInfo(int color)
{
    int tempX = log_x / 10;
    int tempY = log_y / 10;
    switch (color)
    {
        case RED_LOADED_ID:
        {
            for (int xj = tempX - 2; xj <= tempX + 2; xj++)
            {
                for (int yi = tempY - 2; yi <= tempY +2; yi++)
                {
                    if (xj < 0 || xj > kDotWidthNum || yi < 0 || yi > kDotHeightNum)
                    {
                        continue;
                    }
                    red_data[yi][xj] = 1;
 ...
 bool Game1_HikaruForFirstTour::findObjectInfo(int color)
 {
    switch (color)
    {
        case RED_LOADED_ID:
        {
            for (int xj = 0; xj <= kDotWidthNum; xj++)
            {
                for (int yi = 0; yi <= kDotHeightNum; yi++)
                {
                    if (xj < 0 || xj > kDotWidthNum || yi < 0 || yi > kDotHeightNum)
                    {
                        continue;
 ...
 void Game1_HikaruForFirstTour::randomDots(void)
 {
     int xj, yi;
     while (true)
     {
         xj = rnd() % kDotWidthNum;
         yi = rnd() % kDotHeightNum;
         if (map_output_data[yi][xj] == 0 || map_output_data[yi][xj] == 5)
         {
             break;
         }
     }
...
int Game1_HikaruForFirstTour::goInArea(int x, int y, int wide_decide_x, int wide_decide_y, int times) {
