...
#define POINT_UNKNOWN 5
#define POINT_SUPERAREA 2
#define POINT_WHITE 4
#define POINT_DEPOSIT 5
#define POINT_SWAMPLAND 1
#define POINT_WALL 3
#define POINT_YELLOW 0
...
void World2_Test1::loop()
{
	...
	// Formatting using threads
	{
		LOG_MESSAGE(FUNCNAME + "(): Start formatting data", MODE_DEBUG);
		...
		for (int yi = start_len; yi < end_len; ++yi)
		{
			...
			if (yi != 0 && yi != kDotHeight - 1)
			{
				...
				char color_num[20];
				int color_pointer = 0; // Maximum value 18(=20-2)
				int color_pointer_limit = 18;
				for (int xj = 1; xj < kDotWidth - 1; ++xj)
				{
					color_pointer = 0;
					// Find the most common floor color around that dot
				...
				// xj == kDotWidth - 1 In case of, only the upper and lower comparison
		...
		LOG_MESSAGE(FUNCNAME + "(): Finish formatting data", MODE_DEBUG)
	}
	...
	{
		//Know where deposit is
		autoSearch(1);
		//Unkown where deposit is 
		autoSearch(2);
	...
	{
		// if (large_process != 1 || next_allowed_go_time[processGoToDots] > Time)
		// 	{
		// 		if (PositionY < 180 && next_allowed_go_time[0] <= Time)
		// 		{
		// 			processGoToDots = 0;
		// 		}
		// 		else if (PositionY >= 180 && next_allowed_go_time[0] <= Time)
		// 		{
		// 			processGoToDots = 1;
		// 		}
		// 		else {
		// 			if (next_allowed_go_time[1] < next_allowed_go_time[0]) {
		// 				processGoToDots = 1;
		// 			}
		// 			else {
		// 				processGoToDots = 0;
	...
	case DEPOSIT_OBJ:
		...
		else if (!BothColorJudge(object_box2))
		{
...
int World2_Test1::GoToDot(int x, int y)
{
...
// int World2_Test1::GoToDots(int x, int y, int wide_decide_x, int wide_decide_y)
// {
...
int World2_Test1::GoToDots(int x, int y, int wide_decide_x, int wide_decide_y)
{
	...
	if (x != prev_x || y != prev_y)
	{
		...
		for (int i = corner_x[0]; i <= corner_x[1]; i++)
		{
			...
			{
				int investigating_dot_id = j * kDotWidth + i;
				if (i <= 0 || i >= kDotWidth - 1 || j <= 0 || j >= kDotHeight - 1)
				{
					continue;
				}
				...
				//yellow or wall or deposit
				if (cospaceMap.getMapInfo(i, j) < POINT_DEPOSIT)
				{
					continue;
				}

				int costs = static_cast<int>(cospaceMap.getMapInfo(i, j) * 100 + rnd() % 10);
				if (option)
				{
					// 移動しないとき
					int k = 30;
					costs += static_cast<int>(pow(abs(i * kCM2DotScale - pos_x) - k, 2) * 100 - pow(abs(j * kCM2DotScale - pos_y) - k, 2) * 100);
				}
				else
				{
					// 移動するとき
					costs -= static_cast<int>(pow(i * kCM2DotScale - pos_x, 2) / 100 - pow(j * kCM2DotScale - pos_y, 2) / 100);
				}
		...
		same_target_border = static_cast<int>(sqrt(pow(pos_x - target_x * kCM2DotScale, 2) + pow(pos_y - target_y * kCM2DotScale, 2)));
		...
		// 		target_x = x;
		// 		target_y = y;
		// 		target_x /= kCM2DotScale;
		// 		target_y /= kCM2DotScale;
		// 		break;
		// 	}
		...
		// 		target_x = 1;
		// 	}
		// 	if(target_x >= kDotWidth - 1) {
		// 		target_x = kDotWidth - 2;
		// 	}
		// 	if(target_y <= 0) {
		// 		target_y = 1;
		// 	}
		// 	if(target_y >= kDotHeight - 1) {
		// 		target_y = kDotHeight - 2;
		// 	}
		// } while(dot[target_y * kDotWidth + target_x].point <= POINT_WALL);
	}
	local_same_target++;
...
void World2_Test1::autoSearch(float parameter)
{
...
World2_Test1::World2_Test1()
{
	...
	pt.print("World2_Test1::World2_Test1() :");
}
void World2_Test1::setup()
{
	pt.start();
	UserGame1::setup();
	...
	pt.print("World2_Test1::setup() :");
}

void World2_Test1::CheckNowDot()
{
...
long World2_Test1::WhereIsMotor(void)
{
...
long World2_Test1::WhereIsColorSensor(void)
{
	LOG_MESSAGE(FUNCNAME + "(): start", MODE_VERBOSE);
	long x, y;
...
int World2_Test1::GoToPosition(int x, int y, int wide_decide_x, int wide_decide_y, int wide_judge_arrived)
{
...
int World2_Test1::isNearTheFloor(CospaceMap::MapInfo color, int x, int y, int cm_radius)
{
...
void World2_Test1::GoToAngle(int angle, int distance)
{
...
void World2_Test1::Dijkstra(void)
{
	// Initialization
	rep(yi, kDotHeight)
	{
...
World2_Test1::CospaceMap::CospaceMap()
{
...
int World2_Test1::CospaceMap::getMapCurvedTimes(int from_x, int from_y, int target_x, int target_y)
{
...
void World2_Test1::saveColorInfo(void)
{
...
void World2_Test1::calculateWallPosition(void)
{
	...
	if (PositionX != -1)
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			// Basically, it will be about 0.7 times the distance from the actual wall, but at least 2 kCM2DotScale must be opened
